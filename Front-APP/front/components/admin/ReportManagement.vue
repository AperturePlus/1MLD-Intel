...
import { listPendingReports, updateReportState } from '@/api/system/report'
...
export default {
  ...
  methods: {
    getCurrentList() {
      return this.currentTab === 'post' ? this.postReports : this.currentTab === 'comment' ? this.commentReports : this.userReports
    },
    fetchReports() {
      listPendingReports().then((res) => {
        this.allReports = (res && res.data) || []
      })
    },
    handleReportAction({ id, action }) {
      ...
      uni.showModal({
        ...
        success: (res) => {
          ...
          updateReportState({ id, state: newState }).then(() => {
